...
import static com.lai.matrix.Config.listItems;
        ...
        database.child("events").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot child : snapshot.getChildren()) {
                    TrafficEvent event = child.getValue(TrafficEvent.class);
...
//                break;
    ...
    public void startCamera() {
        ...
        startActivityForResult(pic, REQUEST_CAPTURE_IMAGE);
            ...
            public void onComplete(DatabaseError databaseError, DatabaseReference databaseReference) {
                if (databaseError != null) {
                    Toast.makeText(getContext(),
                            "The event is failed, please check your network status.", Toast.LENGTH_SHORT).show();
                    dialog.dismiss();
                } else {
    ...
    public static MainFragment newInstance() {
        ...
        Bundle args = new Bundle();//fragment会有bundle
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        this.googleMap.setMapStyle(MapStyleOptions.loadRawResourceStyle(getActivity(), R.raw.style));

        double latitude = 17.385044;
        double longitude = 78.486671;
        locationTracker = new LocationTracker(getActivity());
        locationTracker.getLocation();
        LatLng location = new LatLng(locationTracker.getLatitude(), locationTracker.getLongitude());

        MarkerOptions marker = new MarkerOptions().position(location).title("You");
...
//        int cy = (int)(fabReport.getY() + fabReport.getHeight() / 2);

//          int cx = (int)fabReport.getX() + 40;
//          int cy = (int)fabReport.getY();

        dialog = ReportDialog.newInstance(getContext(), cx, cy, this, label, prefillText);//这个clas impelemts dialogcallback， dialogcallback的实体，所以可以用this
...
//            }).start();
            new AsyncTask<Void, Void, Bitmap>() {
                @Override
                //It means that zero or more void objects may be passed as the argument(s) for that method.
                ...
                @Override
                protected void onPostExecute(Bitmap bitmap) {//doInBackground的结果是这个method的参数
